...
function TabButton({ active, icon: Icon, children, onClick, badge }: {
  ...
  return (
    <button onClick={onClick} className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium -mb-px border-b-2 transition-colors ${active ? 'border-[#FF6B35] text-[var(--text-strong)]' : 'border-transparent text-[var(--text-muted)] hover:text-[var(--text-body)]'}`}>
      <Icon size={14} />
      {children}
      {badge != null && badge > 0 && (
...
function PlaceholderTab({ label }: { label: string }) {
  return <div className="flex items-center justify-center py-20 text-[var(--text-muted)] text-sm">{label} — próximamente</div>
}
  ...
  if (summary) {
    return (
      <div className="space-y-4 max-w-lg">
        <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[var(--surface-sunken)] rounded-xl p-3">
              <p className="text-[var(--text-muted)] text-xs mb-1">Productos creados</p>
              <p className="text-[var(--text-strong)] text-xl font-bold">{summary.creados}</p>
            </div>
            <div className="bg-[var(--surface-sunken)] rounded-xl p-3">
              <p className="text-[var(--text-muted)] text-xs mb-1">Productos actualizados</p>
              <p className="text-[var(--text-strong)] text-xl font-bold">{summary.actualizados}</p>
            </div>
            <div className="bg-[var(--surface-sunken)] rounded-xl p-3">
              <p className="text-[var(--text-muted)] text-xs mb-1">Recetas procesadas</p>
              <p className="text-[var(--text-strong)] text-xl font-bold">{summary.recetas_procesadas}</p>
            </div>
            <div className="bg-[var(--surface-sunken)] rounded-xl p-3">
        ...
        <button onClick={reset} className="flex items-center gap-2 px-4 py-2 text-sm bg-[var(--surface-sunken)] text-[var(--text-strong)] rounded-lg hover:bg-[var(--surface-sunken)] transition-colors">
      ...
      {!preview && (
        <div className="space-y-3">
          <div
            ...
            className={`border-2 border-dashed rounded-2xl p-10 flex flex-col items-center justify-center gap-3 cursor-pointer transition-colors ${dragging ? 'border-[#FF6B35]/60 bg-[#FF6B35]/5' : 'border-[var(--border-subtle)] hover:border-[var(--border-subtle)] bg-[var(--surface-sunken)]'}`}
          >
            <Upload size={28} className={dragging ? 'text-[#E55A2B]' : 'text-[var(--text-muted)]'} />
            {file ? (
              <div className="text-center">
                <p className="text-[var(--text-strong)] text-sm font-medium">{file.name}</p>
                <p className="text-[var(--text-muted)] text-xs mt-0.5">{(file.size / 1024).toFixed(0)} KB</p>
              </div>
            ) : (
              <div className="text-center">
                <p className="text-[var(--text-muted)] text-sm">Arrastra tu archivo aquí o haz clic para seleccionar</p>
                <p className="text-[var(--text-muted)] text-xs mt-1">Formatos aceptados: .xlsx, .csv — máximo 10 MB</p>
              </div>
            )}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm bg-[#FF6B35] text-white rounded-lg hover:bg-[#FF6B35] disabled:opacity-40 transition-colors"
            >
              <Upload size={14} />
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm bg-[var(--surface-sunken)] text-[var(--text-body)] rounded-lg hover:bg-[var(--surface-sunken)] hover:text-[var(--text-strong)] transition-colors border border-[var(--border-subtle)]"
            >
              Descargar plantilla
            </button>
          </div>
      ...
      {preview && (
        <div className="space-y-5">
          ...
          {preview.productos_preview.length > 0 && (
            <div>
              <h3 className="text-[var(--text-muted)] text-xs font-semibold uppercase tracking-wider mb-2">
                Productos detectados ({preview.productos_preview.length})
              </h3>
              <div className="bg-[var(--surface-sunken)] rounded-2xl border border-[var(--border-subtle)] overflow-x-auto">
                <table className="w-full text-sm min-w-[500px]">
                  <thead>
                    <tr className="border-b border-[var(--border-subtle)]">
                      <th className="text-left text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Nombre</th>
                      <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Unidad</th>
                      <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Cantidad</th>
                      <th className="text-right text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Costo</th>
                    </tr>
                  </thead>
                  <tbody>
                    {preview.productos_preview.map((p, i) => (
                      <tr key={i} className={i < preview.productos_preview.length - 1 ? 'border-b border-[var(--border-subtle)]' : ''}>
                        <td className="px-4 py-2.5 text-[var(--text-strong)]">{p.nombre}</td>
                        <td className="px-3 py-2.5 text-[var(--text-muted)]">{p.unidad}</td>
                        <td className="px-3 py-2.5 text-right text-[var(--text-body)] font-mono">{p.cantidad}</td>
                        <td className="px-4 py-2.5 text-right text-[var(--text-body)]">{formatCurrency(p.costo_por_unidad)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div>
              ...
              <div className="bg-[var(--surface-sunken)] rounded-2xl border border-[var(--border-subtle)] overflow-x-auto">
                <table className="w-full text-sm min-w-[400px]">
                  <thead>
                    <tr className="border-b border-[var(--border-subtle)]">
                      <th className="text-left text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Preparación</th>
                      <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Producto</th>
                      <th className="text-right text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Cantidad</th>
                    </tr>
                  </thead>
                  <tbody>
                    {preview.recetas_preview.map((r, i) => (
                      <tr key={i} className={i < preview.recetas_preview.length - 1 ? 'border-b border-[var(--border-subtle)]' : ''}>
                        <td className="px-4 py-2.5 text-[var(--text-strong)]">{r.preparacion}</td>
                        <td className="px-3 py-2.5 text-[var(--text-muted)]">{r.producto}</td>
                        <td className="px-4 py-2.5 text-right text-[var(--text-body)] font-mono">{r.cantidad}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button onClick={reset} className="px-4 py-2 text-sm text-[var(--text-muted)] hover:text-[var(--text-strong)] transition-colors">
              Cancelar
            </button>
          </div>
        </div>
...
function FotoTab({ restaurantId }: { restaurantId: string }) {
  ...
  if (done) {
    ...
    return (
      <div className="space-y-4 max-w-lg">
        ...
        <button onClick={reset} className="flex items-center gap-2 px-4 py-2 text-sm bg-[var(--surface-sunken)] text-[var(--text-strong)] rounded-lg hover:bg-[var(--surface-sunken)] transition-colors" style={{ minHeight: 44 }}>
    ...
    return (
      <div className="space-y-4 max-w-4xl">
        <div className="rounded-2xl border border-[#FF6B35]/30 bg-[#FF6B35]/5 p-4 flex items-start gap-3">
          <Sparkles size={16} className="text-[#E55A2B] mt-0.5 shrink-0" />
        ...
        {/* Tabla editable */}
        <div className="rounded-2xl border border-[var(--border-subtle)] overflow-x-auto">
          <div className="grid grid-cols-[1.8fr_0.8fr_0.8fr_1fr_1fr_40px] gap-2 bg-[var(--surface-sunken)] px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] min-w-[680px]">
            <div>Producto</div>
            <div>Cantidad</div>
            <div>Unidad</div>
            <div>Costo (CLP)</div>
          ...
          {items.map((it, i) => (
            <div key={i} className="grid grid-cols-[1.8fr_0.8fr_0.8fr_1fr_1fr_40px] gap-2 px-3 py-2 items-center border-t border-[var(--border-subtle)] min-w-[680px]">
              <input
                type="text"
                value={it.name}
                onChange={e => updateItem(i, { name: e.target.value })}
                className="bg-[var(--surface-sunken)] border border-[var(--border-subtle)] rounded px-2 py-1.5 text-sm text-[var(--text-strong)] focus:outline-none focus:border-[#FF6B35]/50"
              />
              <input
                type="number"
                inputMode="decimal"
              ...
              <input
                ...
                className={`bg-[var(--surface-sunken)] border rounded px-2 py-1.5 text-sm text-[var(--text-strong)] focus:outline-none focus:border-[#FF6B35]/50 font-mono ${
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={reset}
            className="px-4 py-2.5 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] text-sm font-semibold hover:bg-[var(--surface-sunken)] transition-colors"
            style={{ minHeight: 44 }}
          >
            Cancelar
          </button>
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-sunken)] p-5 space-y-4">
        <div className="flex items-start gap-3">
          <div className="rounded-lg p-2" style={{ background: 'rgba(255,107,53,0.15)' }}>
            <Camera size={18} className="text-[#E55A2B]" />
          </div>
          <div className="flex-1">
            <p className="text-[var(--text-strong)] font-semibold text-sm">Subí una foto o PDF de la boleta</p>
        ...
        {preview && (
          ...
          <img
            src={preview}
            alt="Vista previa"
            className="rounded-xl border border-[var(--border-subtle)] max-h-64 mx-auto"
          />
        )}
        {file && !preview && (
          <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-sunken)] p-3 flex items-center gap-2 text-sm text-[var(--text-body)]">
        ...
        <label className="block">
          ...
          />
          <span
            className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed border-[var(--border-subtle)] hover:border-[#FF6B35]/50 cursor-pointer text-[var(--text-body)] hover:text-[var(--text-strong)] text-sm font-semibold transition-colors"
            style={{ minHeight: 56 }}
          >
            <Upload size={14} />
...
function StatusBadge({ item }: { item: StockItem }) {
  ...
  if (days !== null) {
    ...
    if (days <= alertDays) {
      return (
        <span
          className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-orange-500/25 text-orange-600"
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-lg shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-subtle)]">
          <h2 className="text-[var(--text-strong)] font-semibold text-sm">{title}</h2>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="space-y-1"><label className="text-[var(--text-muted)] text-xs">{label}</label>{children}</div>
}

const inputCls = 'w-full bg-[var(--surface-sunken)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-[var(--text-strong)] text-sm placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[#FF6B35]/50'
    ...
    <Modal title={isEdit ? 'Editar producto' : 'Nuevo producto'} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="pt-2 border-t border-[var(--border-subtle)]">
          <p className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)] mb-2">Trazabilidad y vencimiento</p>
          <div className="grid grid-cols-2 gap-3">
...
function InventarioTab({ restaurantId }: { restaurantId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16 text-[var(--text-muted)] text-sm">Cargando…</div>
      ) : grouped.size === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-[var(--text-muted)]">
          ...
          {!filterBelowMin && <button onClick={() => setModal({ type: 'create' })} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium bg-[#FF6B35] text-white hover:bg-[#FF6B35] transition-colors"><Plus size={12} />Agregar primer producto</button>}
        </div>
      ) : (
        <div className="space-y-6">
          {Array.from(grouped.entries()).map(([category, catItems]) => (
            <div key={category}>
              <h3 className="text-[var(--text-muted)] text-xs font-semibold uppercase tracking-wider mb-2 px-1">{category}</h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[var(--border-subtle)]">
                      ...
                      <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Unidad</th>
                      <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Cantidad</th>
                      <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Mínimo</th>
                      <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Costo/u</th>
                      <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Valor total</th>
                      <th className="text-center text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Estado</th>
                      <th className="text-right text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Acciones</th>
                    </tr>
                  </thead>
                  <tbody>
...
function MovimientosTab({ restaurantId }: { restaurantId: string }) {
  ...
  return (
    <div className="space-y-4">
      {/* Filters */}
      <div className="bg-[var(--surface-sunken)] border border-[var(--border-subtle)] rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="space-y-1">
          <label className="text-[var(--text-muted)] text-xs">Producto</label>
        ...
        <div className="space-y-1">
          <label className="text-[var(--text-muted)] text-xs">Tipo de movimiento</label>
        ...
        <div className="space-y-1">
          <label className="text-[var(--text-muted)] text-xs">Desde</label>
          <input type="date" className={inputCls} value={filters.from_date} onChange={setFilter('from_date')} />
        </div>
        <div className="space-y-1">
          <label className="text-[var(--text-muted)] text-xs">Hasta</label>
      ...
      {loading ? (
        ...
        <div className="bg-[var(--surface-sunken)] rounded-2xl border border-[var(--border-subtle)] overflow-x-auto">
          <table className="w-full text-sm min-w-[700px]">
            <thead>
              <tr className="border-b border-[var(--border-subtle)]">
                <th className="text-left text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Fecha</th>
                <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Producto</th>
                <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Tipo</th>
                <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Delta</th>
                <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Valor</th>
                <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Razón</th>
                <th className="text-left text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Usuario</th>
              </tr>
            </thead>
            <tbody>
              {movements.map((m, idx) => {
                ...
                return (
                  <tr key={m.id} className={`${idx < movements.length - 1 ? 'border-b border-[var(--border-subtle)]' : ''} hover:bg-[var(--surface-sunken)] transition-colors`}>
                    <td className="px-4 py-3 text-[var(--text-muted)] text-xs whitespace-nowrap">{formatDateTime(m.logged_at)}</td>
                    ...
                    <td className="px-3 py-3 text-[var(--text-muted)] text-xs">{m.reason ?? '—'}</td>
                    <td className="px-4 py-3 text-[var(--text-muted)] text-xs font-mono">{m.logged_by ? m.logged_by.slice(0, 8) + '…' : '—'}</td>
                  </tr>
                )
              })}
            </tbody>
...
function NuevaOrdenModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto">
      <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-2xl shadow-2xl my-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-subtle)]">
          <h2 className="text-[var(--text-strong)] font-semibold text-sm">Nueva orden de compra</h2>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-strong)] transition-colors"><X size={16} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[var(--text-muted)] text-xs font-semibold uppercase tracking-wider">Ítems</p>
              <button type="button" onClick={addItem} className="flex items-center gap-1 text-xs text-[#E55A2B] hover:text-[#E55A2B] transition-colors">
            ...
            <div className="space-y-2">
              {items.map((item, idx) => (
                <div key={idx} className="grid grid-cols-[1fr_100px_100px_32px] gap-2 items-end">
                  <div className="space-y-1">
                    {idx === 0 && <label className="text-[var(--text-muted)] text-xs">Producto</label>}
                    <select
                      className={inputCls}
                  ...
                  <div className="space-y-1">
                    {idx === 0 && <label className="text-[var(--text-muted)] text-xs">Cantidad</label>}
                    <input
                      className={inputCls}
                      type="number"
                      step="0.001"
                  ...
                  <div className="space-y-1">
                    {idx === 0 && <label className="text-[var(--text-muted)] text-xs">Costo/u (CLP)</label>}
                    <input
                      className={inputCls}
                      type="number"
                      step="1"
...
function GenerarFacturaModal({
  ...
  return (
    <Modal title={`Generar factura — ${order.supplier}`} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        <p className="text-[var(--text-muted)] text-xs">Monto total: <span className="text-[var(--text-strong)] font-medium">{formatCurrency(total)}</span></p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="space-y-1">
          <label className="text-[var(--text-muted)] text-xs">Estado</label>
          <select
        ...
        <div className="space-y-1">
          <label className="text-[var(--text-muted)] text-xs">Proveedor</label>
          <input
        ...
        <div className="ml-auto">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium bg-[#FF6B35] text-white hover:bg-[#FF6B35] transition-colors"
          >
      ...
      {ordersLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-[var(--text-muted)]">
          ...
          <button
            onClick={() => setShowNuevaOrden(true)}
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium bg-[#FF6B35] text-white hover:bg-[#FF6B35] transition-colors"
          >
        ...
        <div className="bg-[var(--surface-sunken)] rounded-2xl border border-[var(--border-subtle)] overflow-x-auto">
          <table className="w-full text-sm min-w-[700px]">
            <thead>
              <tr className="border-b border-[var(--border-subtle)]">
                <th className="text-left text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Fecha</th>
                <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Proveedor</th>
                <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Estado</th>
                <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Total estimado</th>
                <th className="text-right text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Acciones</th>
              </tr>
            </thead>
            <tbody>
              {orders.map((order, idx) => (
                <tr key={order.id} className={`${idx < orders.length - 1 ? 'border-b border-[var(--border-subtle)]' : ''} hover:bg-[var(--surface-sunken)] transition-colors`}>
                  <td className="px-4 py-3 text-[var(--text-muted)] text-xs whitespace-nowrap">{formatDate(order.created_at)}</td>
                  <td className="px-3 py-3 text-[var(--text-strong)] font-medium">{order.supplier}</td>
                  <td className="px-3 py-3"><OrderStatusBadge status={order.status} /></td>
                  <td className="px-3 py-3 text-right text-[var(--text-body)]">{formatCurrency(calcOrderTotal(order.items))}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {order.status === 'recibida' && (
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-medium bg-[var(--surface-sunken)] text-[var(--text-muted)] hover:bg-[var(--surface-sunken)] transition-colors"
                        >
      ...
      <div className="space-y-3 pt-2">
        <div className="flex items-center justify-between">
          <h3 className="text-[var(--text-muted)] text-xs font-semibold uppercase tracking-wider flex items-center gap-2">
        ...
        {invoicesLoading ? (
          <div className="flex items-center justify-center py-10 text-[var(--text-muted)] text-sm">Cargando facturas…</div>
        ) : invoices.length === 0 ? (
            ...
            <table className="w-full text-sm min-w-[650px]">
              <thead>
                <tr className="border-b border-[var(--border-subtle)]">
                  <th className="text-left text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Fecha</th>
                  <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Proveedor</th>
                  <th className="text-left text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Número</th>
                  <th className="text-right text-[var(--text-muted)] text-xs font-medium px-3 py-2.5">Monto</th>
                  <th className="text-left text-[var(--text-muted)] text-xs font-medium px-4 py-2.5">Estado</th>
                </tr>
              </thead>
              <tbody>
                {invoices.map((inv, idx) => (
                  <tr key={inv.id} className={`${idx < invoices.length - 1 ? 'border-b border-[var(--border-subtle)]' : ''} hover:bg-[var(--surface-sunken)] transition-colors`}>
                    <td className="px-4 py-3 text-[var(--text-muted)] text-xs whitespace-nowrap">
                      {inv.issued_at ? formatDate(inv.issued_at) : '—'}
                    </td>
                    <td className="px-3 py-3 text-[var(--text-strong)] font-medium">{inv.supplier ?? '—'}</td>
                    <td className="px-3 py-3 text-[var(--text-muted)] font-mono text-xs">{inv.invoice_number ?? '—'}</td>
                    <td className="px-3 py-3 text-right text-[var(--text-body)]">{formatCurrency(inv.total_amount)}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${
...
export default function StockPage() {
  ...
  return (
    <div className="p-6 space-y-5 max-w-6xl">
      <div>
        <h1 className="text-[var(--text-strong)] text-xl font-bold">Control de Stock</h1>
        <p className="text-[var(--text-muted)] text-sm mt-0.5">Inventario, movimientos, importación y órdenes de compra</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[var(--surface-sunken)] rounded-2xl p-4 border border-[var(--border-subtle)]">
          <p className="text-[var(--text-muted)] text-xs mb-1">Valor del inventario</p>
          <p className="text-[var(--text-strong)] text-2xl font-bold">{loading ? '—' : formatCurrency(totalInventoryValue)}</p>
        </div>
