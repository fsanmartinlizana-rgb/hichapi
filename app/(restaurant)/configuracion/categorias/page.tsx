    ...
    <div className="p-6 space-y-6 max-w-5xl">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[var(--text-strong)] text-xl font-bold flex items-center gap-2">
            <BookOpen size={20} className="text-[#E55A2B]" /> Categorías y ruteo
          </h1>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <form onClick={e => e.stopPropagation()} onSubmit={createCategory} className="w-full max-w-md bg-[var(--bg-canvas)] border border-[var(--border-subtle)] rounded-2xl p-6 space-y-4">
            <h3 className="text-[var(--text-strong)] font-bold text-lg flex items-center gap-2">
              <BookOpen size={18} className="text-[#E55A2B]" /> Nueva categoría
            </h3>
            ...
            {brandId && (
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                />
                <div>
                  <p className="text-[var(--text-strong)] text-xs font-medium">Compartir con todos los locales de la marca</p>
                  <p className="text-[var(--text-muted)] text-[10px]">Si la destildas, esta categoría solo existe en el local actual.</p>
                </div>
              </label>
            )}
    ...
    <div className="bg-white/[0.02] border border-[var(--border-subtle)] rounded-2xl overflow-hidden">
      <div className="flex items-center gap-3 p-4">
        <div className="w-10 h-10 rounded-xl bg-[#FF6B35]/10 border border-[#FF6B35]/20 flex items-center justify-center">
          <BookOpen size={16} className="text-[#E55A2B]" />
        </div>
      ...
      {editing && (
        <div className="px-4 pb-4 space-y-4 border-t border-[var(--border-subtle)] pt-4">
          {stations.length === 0 ? (
            <p className="text-[var(--text-muted)] text-xs italic">No hay estaciones creadas. Creá al menos una en Configuración → Estaciones.</p>
          ) : (
            <>
              <p className="text-[var(--text-muted)] text-xs font-medium">Estaciones que reciben esta categoría:</p>
              <div className="space-y-3">
                {[...byLocation.entries()].map(([locName, sts]) => (
                  <div key={locName}>
                    <p className="text-[var(--text-muted)] text-[10px] uppercase tracking-wide mb-1.5 flex items-center gap-1">
                            ...
                            className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs transition-colors text-left ${
                              checked
                                ? isPrim
                                  ...
                                  : 'bg-[var(--surface-sunken)] border-[var(--border-subtle)] text-[var(--text-strong)]'
                          ...
                          >
                            ...
                            {checked && (
                              <span
                                ...
                                className={`text-[9px] px-1 py-0.5 rounded ${isPrim ? 'bg-[#FF6B35] text-black' : 'bg-[var(--surface-sunken)] text-[var(--text-muted)] hover:bg-[var(--surface-sunken)]'}`}
                              >
              ...
              <p className="text-[var(--text-muted)] text-[10px]">
                Marcá con &quot;Set&quot; para elegir la station <span className="text-[#E55A2B]">principal</span> donde se prepara el plato (las otras reciben notificación).
              </p>
            </>
          )}
          ...
          <div className="flex gap-2 justify-end pt-1">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-muted)] text-xs hover:bg-[var(--surface-sunken)] transition-colors"
            >
              <X size={12} /> Cancelar
            </button>
            <button
              onClick={save}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[#FF6B35] text-black text-xs font-bold hover:bg-[#FF6B35] transition-colors"
            >
