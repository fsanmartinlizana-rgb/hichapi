  ...
  return (
    <div className="p-6 space-y-6 max-w-3xl">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[var(--text-strong)] text-xl font-bold">Tono de Chapi</h1>
          <p className="text-[var(--text-muted)] text-sm mt-0.5">Personaliza cómo Chapi habla con tus clientes</p>
        </div>
      ...
      <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl p-5 space-y-4">
        <p className="text-[var(--text-strong)] font-semibold text-sm">Personalidad base</p>
          ...
          {PRESETS.map(p => (
            <button key={p.id} onClick={() => applyPreset(p.id)}
              ...
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <p className={`font-semibold text-sm ${config.preset === p.id ? 'text-[#E55A2B]' : 'text-[var(--text-body)]'}`}>
                  {p.label}
                </p>
                {config.preset === p.id && <Check size={12} className="text-[#E55A2B] ml-auto" />}
              </div>
        ...
        {/* Example */}
        <div className="bg-[var(--surface-sunken)] border border-[var(--border-subtle)] rounded-xl p-3.5">
          <p className="text-[var(--text-muted)] text-[10px] mb-2 font-medium uppercase tracking-wide">Ejemplo con este tono:</p>
          <div className="flex gap-2.5">
            ...
            <p className="text-[var(--text-muted)] text-sm italic leading-relaxed">"{activePreset.example}"</p>
          </div>
        </div>
      </div>
      ...
      {/* Fine-tuning */}
      <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl p-5 space-y-4">
        <p className="text-[var(--text-strong)] font-semibold text-sm">Ajustes finos</p>

        <div className="grid grid-cols-3 gap-3">
          {/* Emojis */}
          <div className="space-y-2">
            <p className="text-[var(--text-muted)] text-xs">Emojis</p>
            <button onClick={() => setConfig(c => ({ ...c, useEmojis: !c.useEmojis }))}
              className={`w-full py-2.5 rounded-xl border text-xs font-medium transition-all
                ${config.useEmojis ? 'bg-[#FF6B35]/15 border-[#FF6B35]/30 text-[#E55A2B]' : 'bg-[var(--surface-sunken)] border-[var(--border-subtle)] text-[var(--text-muted)]'}`}>
              {config.useEmojis ? '😊 Activados' : '— Desactivados'}
            </button>
          </div>
          {/* Response length */}
          <div className="space-y-2">
            <p className="text-[var(--text-muted)] text-xs">Largo respuesta</p>
              ...
              {(['corta', 'media', 'larga'] as const).map(l => (
                <button key={l} onClick={() => setConfig(c => ({ ...c, responseLength: l }))}
                  className={`flex-1 py-2.5 rounded-xl border text-[10px] capitalize transition-all
                    ${config.responseLength === l ? 'bg-[#FF6B35]/15 border-[#FF6B35]/30 text-[#E55A2B]' : 'bg-[var(--surface-sunken)] border-[var(--border-subtle)] text-[var(--text-muted)]'}`}>
                  {l}
                </button>
              ))}
            </div>
          ...
          {/* Language */}
          <div className="space-y-2">
            <p className="text-[var(--text-muted)] text-xs">Variante idioma</p>
            <div className="flex gap-1">
              {LANGUAGE_OPTIONS.map(l => (
                <button key={l.id} onClick={() => setConfig(c => ({ ...c, language: l.id as ToneConfig['language'] }))}
                  className={`flex-1 py-2.5 rounded-xl border text-[10px] transition-all
                    ${config.language === l.id ? 'bg-[#FF6B35]/15 border-[#FF6B35]/30 text-[#E55A2B]' : 'bg-[var(--surface-sunken)] border-[var(--border-subtle)] text-[var(--text-muted)]'}`}>
                  {l.label}
                </button>
              ))}
            </div>
        ...
        <div className="space-y-3">
          <p className="text-[var(--text-muted)] text-xs font-medium">Frases personalizadas</p>
          {[
            ...
            <div key={key} className="space-y-1.5">
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-4 py-2.5 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] text-xs placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[#FF6B35]/40 resize-none transition-colors"
              />
            </div>
          ))}
        </div>
          ...
          <>
            ...
            {Object.entries(
              ...
              <div key={cat} className="space-y-1.5">
                <p className="text-[var(--text-muted)] text-[10px] uppercase tracking-widest font-semibold">{cat}</p>
                <div className="flex flex-wrap gap-2">
                  {items.map(item => {
            ...
            {featuredIds.length >= 3 && (
              <p className="text-[var(--text-muted)] text-[10px]">Máximo 3 platos seleccionados. Quita uno para agregar otro.</p>
            )}
          </>
        )}
      </div>
      ...
      <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl p-5 space-y-4">
        <div className="flex items-center gap-2">
          <Play size={14} className="text-[#E55A2B]" />
          <p className="text-[var(--text-strong)] font-semibold text-sm">Probar el tono</p>
        </div>
          ...
          {PREVIEW_SCENARIOS.map((s, i) => (
            <button key={i} onClick={() => { setActiveScenario(i); setTestInput(s.user); setTestOutput('') }}
              className={`text-xs px-3 py-1.5 rounded-full border transition-all
                ${activeScenario === i && testInput === s.user ? 'bg-[#FF6B35]/15 border-[#FF6B35]/30 text-[#E55A2B]' : 'bg-[var(--surface-sunken)] border-[var(--border-subtle)] text-[var(--text-muted)] hover:border-[var(--border-subtle)]'}`}>
              "{s.user}"
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          <input value={testInput} onChange={e => setTestInput(e.target.value)}
            ...
            className="flex-1 px-4 py-2.5 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] text-sm placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[#FF6B35]/50 transition-colors" />
        ...
        {testOutput && (
          <div className="flex gap-2.5 bg-[var(--surface-sunken)] border border-[var(--border-subtle)] rounded-xl p-4">
            <div className="w-6 h-6 rounded-full bg-[#FF6B35] flex items-center justify-center text-white text-[9px] font-bold shrink-0">C</div>
            <div>
              <p className="text-[var(--text-muted)] text-[9px] mb-1.5 font-medium">CHAPI RESPONDE:</p>
              <p className="text-[var(--text-body)] text-sm leading-relaxed">{testOutput}</p>
            </div>
          </div>
        )}
      </div>
