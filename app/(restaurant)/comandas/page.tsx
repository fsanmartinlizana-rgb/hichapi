...
const STATION_FILTERS: { value: StationFilter; label: string; icon: typeof ChefHat; color: string }[] = [
  ...
  { value: 'cocina', label: 'Cocina', icon: ChefHat, color: '#B45309' },
  { value: 'barra',  label: 'Barra',  icon: Wine,    color: '#6D28D9' },
]

const DEST_META: Record<Destination, { label: string; color: string }> = {
  cocina:  { label: 'Cocina', color: '#B45309' },
  barra:   { label: 'Barra',  color: '#6D28D9' },
...
function Toast({ msg }: { msg: ToastMsg }) {
  ...
  return (
    <div className={`flex items-center gap-2 border text-[var(--text-body)]
                    text-xs px-4 py-2.5 rounded-xl shadow-xl animate-fade-in
                    ${isStock
                      ? 'bg-[var(--surface-card)] border-amber-500/30'
                      : isInfo
                        ? 'bg-[var(--surface-card)] border-emerald-500/30'
      ...
      <div className="flex items-center justify-between px-4 py-2.5">
            ...
            <span className={`text-xs font-medium ${hasOut ? 'text-red-700' : 'text-amber-700'}`}>
              ...
              {lowItems.map(([name, e], idx) => (
                <span key={name}>
                  ...
                  {e.qty !== undefined && (
                    <span className="text-[var(--text-muted)]"> ({e.qty} unds.)</span>
                  )}
                </span>
              ))}
        ...
        <div className="flex items-center gap-2 shrink-0 ml-3">
          ...
          <button
            ...
            className="text-[var(--text-muted)] hover:text-[var(--text-muted)] transition-colors p-0.5"
...
function StockQtyPopover({
  ...
  return (
    <div
      className="absolute right-0 top-6 z-20 bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-xl shadow-2xl p-3 w-48 flex flex-col gap-2"
      onClick={e => e.stopPropagation()}
    >
      <p className="text-[var(--text-muted)] text-[10px] leading-tight">
        Pocas unidades de{' '}
        <span className="text-[var(--text-body)] font-semibold">{itemName}</span>
      </p>
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 bg-[var(--surface-sunken)] border border-[var(--border-subtle)] rounded-lg px-2 py-1 text-[var(--text-strong)] text-sm text-center focus:outline-none focus:border-amber-400/50 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none"
        />
        <span className="text-[var(--text-muted)] text-[10px]">unds.</span>
      </div>
      <div className="flex gap-1.5">
        ...
        <button
          onClick={onClose}
          className="px-2 py-1 rounded-lg text-[11px] text-[var(--text-muted)] hover:text-[var(--text-muted)] transition-colors"
        >
          Cancelar
        </button>
      </div>
...
function DevolucionPopover({
  ...
  return (
    <div
      className="absolute right-0 top-6 z-20 bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-xl shadow-2xl p-3 w-52 flex flex-col gap-2"
      onClick={e => e.stopPropagation()}
    >
      <p className="text-[var(--text-muted)] text-[10px] leading-tight">
        Devolución de{' '}
      ...
      <select
        ...
        className="w-full px-2 py-1.5 rounded-lg bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] text-[11px] focus:outline-none focus:border-orange-400/50 appearance-none"
      >
        {DEVOLUCION_REASONS.map(r => (
          <option key={r} value={r} className="bg-[var(--surface-card)]">{r}</option>
        ))}
      </select>
      <div className="flex gap-1.5">
        <button
          onClick={() => onConfirm(reason)}
          className="flex-1 py-1 rounded-lg text-[11px] font-semibold bg-orange-500/20 text-orange-600 hover:bg-orange-500/30 transition-colors"
        >
          Confirmar
        </button>
        <button
  ...
  return (
    <div className={`rounded-xl border p-3.5 space-y-3 transition-all
                     ${order.crossFrom
                       ...
                       : 'bg-[var(--surface-card)] ' + (urgent ? 'border-red-500/40' : 'border-[var(--border-subtle)]')}`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[var(--text-strong)] text-xs font-semibold">
      ...
      <div className="space-y-1">
        {visibleItems.map(({ it: item, i }) => {
          ...
          return (
            ...
            >
              <span className="text-[var(--text-muted)] text-[10px] w-3 shrink-0 mt-0.5">{item.qty}×</span>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <span className={`text-xs transition-all ${
                    devuelto
                      ? 'line-through text-orange-600/50'
                      : display === '86'
                  ...
                  {/* devuelto badge */}
                  {devuelto && (
                    <span className="text-[8px] font-bold px-1 py-0.5 rounded bg-orange-500/20 text-orange-600 border border-orange-500/30 shrink-0 flex items-center gap-0.5">
                      <RotateCcw size={7} />
                      devuelto
                    </span>
                  )}
              ...
              {order.status === 'entregada' && !devuelto && (
                <div className="relative shrink-0">
                  <button
                    ...
                    className="w-5 h-5 rounded flex items-center justify-center text-orange-600/60 hover:text-orange-600 hover:bg-orange-500/15 transition-all"
                    title="Devolución"
                  >
                    <RotateCcw size={10} />
                  </button>
      ...
      {order.status === 'entregada' && (role === 'garzon' || role === 'admin') && onCharge && (
        <button
          ...
          className="w-full py-1.5 rounded-lg text-[11px] font-semibold transition-colors flex items-center justify-center gap-1.5
                     bg-[#FF6B35]/18 text-[#E55A2B] border border-[#FF6B35]/30 hover:bg-[#FF6B35]/28"
        >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-subtle)]">
          <div>
            <h2 className="text-[var(--text-strong)] font-bold text-base">Nueva comanda</h2>
            <p className="text-[var(--text-muted)] text-xs mt-0.5">Crea una comanda manualmente</p>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-lg bg-[var(--surface-sunken)] hover:bg-[var(--surface-sunken)] flex items-center justify-center text-[var(--text-muted)] transition-colors"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-[var(--text-muted)] text-xs font-medium">Mesa</label>
              <div className="relative">
                <select
                  value={mesa}
                  onChange={e => setMesa(e.target.value)}
                  className="w-full pl-3 pr-9 py-2.5 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] text-sm focus:outline-none focus:border-[#FF6B35]/50 transition-colors appearance-none cursor-pointer"
                >
                  <option value="" className="bg-[var(--surface-card)]">
                    {tables.length === 0 ? 'Sin mesas — créalas en /mesas' : 'Seleccionar…'}
                  </option>
                  {tables.map(t => (
                    <option key={t.id} value={t.label} className="bg-[var(--surface-card)]">{t.label}</option>
                  ))}
                </select>
            ...
            <div className="space-y-1.5">
              <label className="text-[var(--text-muted)] text-xs font-medium">Personas</label>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setPax(p => Math.max(1, p - 1))}
                  className="w-9 h-9 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] hover:bg-[var(--surface-sunken)] transition-colors flex items-center justify-center font-bold"
                >−</button>
                <span className="flex-1 text-center text-[var(--text-strong)] font-bold text-base">{pax}</span>
                <button
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-[var(--text-muted)] text-xs font-medium">Platos</label>
              <button
                onClick={addLine}
                ...
                <div key={i} className="space-y-1.5">
                  <div className="flex gap-2">
                    ...
                    <select
                      ...
                      className="w-14 px-2 py-2 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] text-sm focus:outline-none focus:border-[#FF6B35]/50 appearance-none text-center cursor-pointer"
                    >
                      {[1,2,3,4,5,6].map(n => <option key={n} value={n} className="bg-[var(--surface-card)]">{n}×</option>)}
                    </select>
                    {/* Item name */}
                    <div className="relative flex-1">
                      <select
                        ...
                        className="w-full pl-3 pr-9 py-2 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] text-sm focus:outline-none focus:border-[#FF6B35]/50 appearance-none cursor-pointer"
                      >
                        <option value="" className="bg-[var(--surface-card)]">
                          {menuItems.length === 0 ? 'Sin platos — créalos en /carta' : 'Seleccionar plato…'}
                        </option>
                        {menuItems.map(item => (
                          <option key={item.name} value={item.name} className="bg-[var(--surface-card)]">{item.name}</option>
                        ))}
                      </select>
                      <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] pointer-events-none" />
                    </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-semibold transition-colors ${line.dest === 'cocina' ? 'bg-[#FF6B35]/20 text-[#E55A2B] border border-[#FF6B35]/30' : 'bg-[var(--surface-sunken)] text-[var(--text-muted)] border border-[var(--border-subtle)] hover:text-[var(--text-muted)]'}`}
                    >
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 rounded-lg bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-body)] placeholder:text-[var(--text-muted)] text-[11px] focus:outline-none focus:border-[var(--border-subtle)]"
                  />
                </div>
              ))}
            </div>
        ...
        <div className="px-5 py-4 border-t border-[var(--border-subtle)] flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl border border-[var(--border-subtle)] text-[var(--text-muted)] text-sm hover:border-[var(--border-subtle)] hover:text-[var(--text-muted)] transition-colors"
          >
            Cancelar
          </button>
          <button
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pt-6 pb-4 flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-[var(--text-strong)] text-xl font-bold">Comandas</h1>
        ...
        <div className="flex items-center gap-2">
          {/* Station filter */}
          <div className="flex items-center gap-1 p-1 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)]">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all select-none
                              ${active ? 'bg-[var(--surface-sunken)] text-[var(--text-strong)] shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text-muted)]'}`}
            ...
            {ROLE_OPTIONS.map(opt => (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-semibold
                            ...
                            ${role === opt.value
                              ? 'bg-[var(--surface-sunken)] text-[var(--text-strong)] shadow-sm'
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-4 py-2 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-strong)] text-sm placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--border-subtle)] w-44"
            />
          </div>
          {(!hasMenu || !hasGarzon) ? (
            <div className="group relative">
              <button
                disabled
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[var(--surface-sunken)] border border-[var(--border-subtle)] text-[var(--text-muted)] text-sm font-semibold transition-colors shadow-sm cursor-not-allowed"
              >
                <Plus size={14} />
                Nueva comanda
              </button>
              <div className="absolute top-full right-0 mt-2 px-3 py-2 bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-lg shadow-xl text-xs text-[var(--text-body)] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3 shrink-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[var(--text-strong)] text-sm font-semibold">{col.label}</span>
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                  {colOrders.length === 0 ? (
                    <div className="border border-dashed border-[var(--border-subtle)] rounded-xl h-24 flex items-center justify-center">
                      <p className="text-[var(--text-muted)] text-xs">Sin comandas</p>
                    </div>
                  ) : (
                    (() => {
                            ...
                            <div className="flex items-center justify-between gap-2 px-1">
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <p className="text-[var(--text-strong)] text-xs font-bold truncate">
                                    {label} · sesión activa
                                  </p>
      ...
      {showNueva && !restId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-6">
          <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl p-6 max-w-sm w-full text-center">
            <RefreshCw size={20} className="text-[#E55A2B] animate-spin mx-auto mb-3" />
            <p className="text-[var(--text-strong)] text-sm font-semibold mb-1">Cargando tu restaurante…</p>
            <p className="text-[var(--text-muted)] text-xs">En un momento podes tomar la comanda.</p>
          </div>
        </div>
      )}
...
function ChargeOrderModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl p-5 w-full max-w-sm" onClick={e => e.stopPropagation()}>
        <h3 className="text-[var(--text-strong)] font-bold text-base mb-1">Cobrar {tableLabel}</h3>
        <p className="text-[var(--text-muted)] text-xs mb-4">Total: <span className="text-[#E55A2B] font-mono font-bold">${amount.toLocaleString('es-CL')}</span></p>

        <p className="text-[var(--text-muted)] text-[10px] uppercase tracking-wider mb-2">Método</p>
        <div className="grid grid-cols-3 gap-1.5 mb-4">
          {(['digital','cash','mixed'] as const).map(m => (
            <button key={m} onClick={() => setMethod(m)}
              className={`py-2 rounded-lg text-[11px] font-medium border transition-colors ${
                method === m ? 'bg-[#FF6B35]/20 border-[#FF6B35]/40 text-[#E55A2B]' : 'bg-[var(--surface-sunken)] border-[var(--border-subtle)] text-[var(--text-muted)] hover:text-[var(--text-body)]'
              }`}>
        ...
        {method === 'mixed' && (
          <div className="mb-4">
            <label className="text-[var(--text-muted)] text-[10px] uppercase tracking-wider mb-1.5 block">Efectivo recibido</label>
            <input type="number" min={0} max={amount} value={cashAmount}
              ...
              className="w-full bg-[var(--surface-sunken)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-[var(--text-strong)] text-sm focus:outline-none focus:border-[#FF6B35]/50" />
            <p className="text-[var(--text-muted)] text-[10px] mt-1">Tarjeta: ${Math.max(0, amount - cashAmount).toLocaleString('es-CL')}</p>
          </div>
        )}
        ...
        <div className="flex gap-2">
          <button onClick={onClose} disabled={saving}
            className="flex-1 py-2 rounded-lg text-[11px] text-[var(--text-muted)] hover:text-[var(--text-body)] transition-colors">
            Cancelar
          </button>
...
function SendToDeliveryModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-[var(--surface-card)] border border-blue-500/25 rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl">
        <div className="flex items-center justify-between">
          <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="text-[var(--text-muted)] text-xs mb-1 block">Dirección de entrega *</label>
            <input
              type="text"
              ...
              autoFocus
              className="w-full bg-[var(--surface-sunken)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-[var(--text-strong)] text-sm focus:outline-none focus:border-blue-400/50"
              required
            />
          </div>
          <div>
            <label className="text-[var(--text-muted)] text-xs mb-1 block">Teléfono del cliente</label>
            <input
              type="tel"
              value={clientPhone}
          ...
          <div>
            <label className="text-[var(--text-muted)] text-xs mb-1 block">Dirección de recogida (restaurante)</label>
            <input
              type="text"
              value={pickupAddress}
