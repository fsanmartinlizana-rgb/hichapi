...
export default function CuentaPedidoDetallePage() {
  ...
  if (!order) {
    return (
      <div className="space-y-4">
        <Link href="/cuenta/pedidos" className="text-[var(--text-muted)] text-sm flex items-center gap-1 hover:text-[var(--text-strong)]">
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold text-[var(--text-strong)]">{order.restaurant_name}</h1>
      ...
      <div className="bg-white/[0.03] border border-[var(--border-subtle)] rounded-2xl p-4 space-y-3">
        <p className="text-[var(--text-strong)] font-semibold text-sm">Resumen</p>
        <p className="text-2xl font-bold text-[var(--text-strong)]">{formatCurrency(order.total_clp)}</p>
      ...
      {order.items && order.items.length > 0 && (
        <div className="bg-white/[0.03] border border-[var(--border-subtle)] rounded-2xl p-4">
          <p className="text-[var(--text-strong)] font-semibold text-sm mb-3">Items</p>
          <ul className="space-y-2">
            {order.items.map((item, i) => (
              <li key={i} className="flex justify-between text-sm">
                ...
                <span className="text-[var(--text-muted)]">{formatCurrency(item.unit_price * item.quantity)}</span>
              </li>
            ))}
          </ul>
        </div>
