...
const RequestSchema = z.object({
  ...
  intent: z
    ...
    .optional(),
  /** Opt-in del usuario: cuando clica "ver alternativas en {zone}", el
   *  frontend reenvía con allow_alternatives=true → se ignora cuisine. */
  allow_alternatives: z.boolean().optional(),
})
...
// ── Búsqueda con caché por intent ────────────────────────────────────────────
async function search(intent: Intent, ignoreCuisine: boolean): Promise<SearchOutput> {
  const cacheKey = intentCacheKey({
    ...intent,
    cuisine_type: ignoreCuisine ? '__alt' : intent.cuisine_type,
  })
  const cached = queryCache.get(cacheKey) as SearchOutput | undefined
  if (cached) return cached

  const out = await searchRestaurants(intent, { ignoreCuisine })
  // Nunca cacheamos vacío: el frontend dispara enrichment async y reintenta
  // con el mismo intent. Si cacheamos [], el retry recibe el cache viejo y
  // no ve los restaurantes recién insertados.
  if (out.results.length > 0) queryCache.set(cacheKey, out)
  return out
}
...
function ruleBasedParser(message: string, prevIntent: Intent) {
  ...
  const ZONES = [
    ...
    'maipu','maipú','la florida','puente alto','la reina','peñalolen','penalolen',
  ...
  // tal cual; canonicalCuisine() del módulo se encarga de mapearla.
  const CUISINE_KEYWORDS = [
    'japonesa','japones','japan','sushi','ramen','italiana','italian','pizza','pasta',
    'peruana','peruano','ceviche','mexicana','mexican','taco','chilena','chileno',
    'cazuela','empanada','vegana','vegano','vegetariana','vegetariano','tailandesa',
    'thai','parrilla','asado','steak','mariscos','marisc','pescado','cafeteria',
    'cafe','brunch','panaderia','pan ','heladeria','helado','india','hindu','hindú',
    'china','chifa','wok','coreana','korean','vietnamita','pho','arabe','árabe',
    'libanesa','turca','shawarma','falafel','kebab','americana','american','bbq',
    'argentina','venezolana','arepa','colombiana','espanola','española','paella',
    'francesa','french','mediterranea','mediterránea','griega','greek','asiatica',
    'asiática','asian','sandwicheria','sandwich','cerveceria','cervecería','pub',
    'saludable','healthy','hamburguesa','burger','hambur',
  ]
  const cuisine_type = CUISINE_KEYWORDS.find(k => msg.includes(k)) ?? prevIntent.cuisine_type ?? null
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { message, intent, allow_alternatives } = RequestSchema.parse(body)
    ...
    ;(async () => {
      try {
        ...
        // Merge intents: lo que Claude devuelva (explícito) gana sobre el
        // contexto previo. Solo si Claude omite un campo, mantenemos el viejo.
        ...
        if (chapiResponse.intent) {
          for (const key of Object.keys(chapiResponse.intent) as Array<keyof Intent>) {
            const v = chapiResponse.intent[key]
            if (v !== undefined) {
              // @ts-ignore
              mergedIntent[key] = v
            }
