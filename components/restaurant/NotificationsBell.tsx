    ...
    <>
      <button
        ...
        className={[
          ...
          panelOpen
            ? 'bg-[#FF6B35]/20 text-[#E55A2B]'
            : 'bg-[var(--surface-sunken)] text-[var(--text-muted)] hover:text-[var(--text-strong)] hover:bg-[var(--surface-sunken)]',
        ].join(' ')}
      >
        <Bell size={14} strokeWidth={2} />
        {unreadCount > 0 && (
          <span
            className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 rounded-full bg-[#FF6B35] text-white text-[9px] font-bold flex items-center justify-center border border-[var(--border-subtle)] shadow-sm shadow-[#FF6B35]/40"
          >
            {display}
          </span>
        )}
