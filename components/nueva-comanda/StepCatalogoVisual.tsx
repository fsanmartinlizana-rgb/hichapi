        ...
        <button
          ...
          className={[
            ...
            active === cat
              ...
              : 'border border-[var(--border-subtle)] text-[var(--text-muted)] hover:border-[var(--border-subtle)] hover:text-[var(--text-body)]',
          ].join(' ')}
        >
          {cat}
        </button>
...
function ProductCard({
  ...
  return (
    ...
    >
      ...
      <span className="mt-1 text-xs font-semibold text-[#E55A2B]">{price}</span>
    </button>
  )
}
  ...
  return (
    <section ref={sectionRef} className="mb-2">
      ...
      >
        ...
        {totalInCart > 0 && (
          <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-[#FF6B35]/20 px-1.5 text-[10px] font-bold text-[#E55A2B]">
            {totalInCart}
          </span>
        )}
      </button>
    ...
    >
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1 flex-shrink-0">
          <button
            ...
            className="flex h-6 w-6 items-center justify-center rounded-full border border-[var(--border-subtle)] bg-[var(--surface-sunken)] text-[var(--text-body)] hover:bg-[var(--surface-sunken)] transition-colors"
            aria-label="Decrementar"
          >
            −
          </button>
          <span className="w-5 text-center font-medium text-[var(--text-strong)]">{line.qty}</span>
          <button
        ...
        <div className="flex flex-1 min-w-0 flex-col">
          ...
          >
            <span className="flex-1 truncate text-xs font-medium text-[var(--text-body)]">{line.name}</span>
            {line.note && (
          ...
          <span className="text-xs font-semibold text-[#E55A2B]">{partialPrice}</span>
        </div>

        <button
          onClick={onRemove}
          className="flex-shrink-0 text-[var(--text-muted)] hover:text-red-700 transition-colors"
      ...
      {expanded && (
        ...
        >
          ...
          <div>
            <label className="mb-1 block text-xs text-[var(--text-muted)]">Destino</label>
                ...
                <button
                  ...
                  className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize transition ${
                    line.destination === dest
                      ...
                      : 'border border-[var(--border-subtle)] text-[var(--text-muted)] hover:bg-[var(--surface-sunken)]'
                  }`}
                >
                  {dest}
                </button>
...
function OrderSummaryPanel({
  ...
  return (
    <div className="flex flex-col h-full">
      <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-[var(--text-muted)]">Resumen</h2>

      {lines.length === 0 ? (
        <p className="flex-1 text-center text-sm text-[var(--text-muted)] py-8">
  ...
  if (menuItems.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 py-16 text-center">
        <p className="text-[var(--text-muted)]">No hay productos disponibles en el menú.</p>
        <a
          href="/carta"
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      >
        ...
        >
          <span className="text-sm text-[var(--text-body)] flex items-center gap-1.5">
            {lines.length === 0
              ? 'Sin productos'
              : (
                <>
                  ...
                  {lines.length > 0 && <ChevronUp size={13} className="text-[#E55A2B]" />}
                </>
              )}
          </span>
        ...
        <div className="fixed inset-0 z-30 lg:hidden">
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-subtle)] shrink-0">
              <h3 className="text-[var(--text-strong)] font-semibold text-sm">Tu pedido</h3>
              <button
                type="button"
                onClick={() => setShowMobileSummary(false)}
                aria-label="Cerrar"
                className="p-2 -mr-2 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-strong)] hover:bg-[var(--surface-sunken)]"
